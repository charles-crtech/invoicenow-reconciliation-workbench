...
|---|---|---|---|---|---|
| `REQ-001` | Import synthetic source records once and retain diagnosable rejects | SCOPE; IRAS-GUIDE for invoice-data context | Import module, invoice module, PostgreSQL, batch UI | Import API tests, Testcontainers idempotency/quarantine tests, manifest reconciliation | In progress; CSV/JSON registration, bounded parsing, accepted persistence, safe quarantine, exact replay, and equivalent 413-unit smoke reconciliation implemented; asynchronous progress/UI pending |
