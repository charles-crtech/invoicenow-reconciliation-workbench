...
| IRW-103 | Implement ledger-entry model | review | 97-test local gate proves debit/credit exclusivity, signed values, observable cutoff mismatch, source uniqueness, V4 constraints, and persistence; PR review pending |
...
IRW-000 through IRW-009 and IRW-100 through IRW-102 are complete. IRW-103 is in
review with its immutable ledger-entry model, Flyway V4, and 97-test local gate
implemented. The application foundation remains on protected `main`; its Java 21
verification, PostgreSQL/Flyway migrations, Testcontainers checks, and supported
Dependabot ecosystems pass. IRW-104 follows the IRW-103 merge.
