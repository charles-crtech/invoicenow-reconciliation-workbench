...
Import-pipeline phase: the verified Java 21/Spring Boot and PostgreSQL foundation includes fixed-precision domain aggregates, versioned JSON/CSV source contracts, and deterministic smoke, demo, and functional-scale data profiles. The import module now provides constrained batch registration, atomic checksum/idempotency decisions, role-protected API operations, and safe problem responses. Bounded byte parsing and quarantine remain the next slices.
...
- [Import-batch registration model](docs/domain/import-batch.md)
